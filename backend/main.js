...
class GameInfo {
    constructor(id, name, search, gameplayMain, gameplayMainExtra, gameplayCompletionist, imageUrl) {
        this.id = id;
        this.name = name;
        this.search = search;
...
function parseResult(search, result) {
...
let gameFetchPromise = function(game, numRetry) {
    ...
    return hltbService
            .search(game)
            .then(result => {
                let gameInfo = parseResult(game, result);
                if (gameInfo != null) {
                    return gameInfo;
                }
